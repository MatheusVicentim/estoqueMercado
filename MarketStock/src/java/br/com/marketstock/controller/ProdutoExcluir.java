    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (sessao != null) {
            ...
            try {
                ...
                Produto oProduto = (Produto) oProdutoDAO.carregaInativaAtiva(nIdProduto);
                //Usado para abrir conexão com BD
                oProdutoDAO = new ProdutoDAO();
                if (oProdutoDAO.excluir(oProduto)) {
                    sMensagem = "Produto " + oProduto.getSituacao() + " com sucesso!";
                } else {
                    sMensagem = "Problemas ao ativar ou excluir Produto. Verifique os dados informados e tente novamente!";
                }
                request.setAttribute("mensagem", sMensagem);
                request.getRequestDispatcher("ProdutoListar?tipoTela=L").forward(request, response);
            } catch (Exception ex) {
