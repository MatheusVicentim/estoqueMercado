 ...
 * @author mathe
 */
@WebServlet(name = "EmbalagemCarregar", urlPatterns = {"/EmbalagemCarregar"})
public class EmbalagemCarregar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (sessao != null) {
            ...
            try {
                ...
                Embalagem oEmbalagem = (Embalagem) oEmbalagemDAO.carregar(nIdEmbalagem);

                Gson gson = new Gson();
                String jsonEmbalagem = gson.toJson(oEmbalagem);
                response.setCharacterEncoding("UTF-8");
                response.getWriter().write(jsonEmbalagem);
            } catch (Exception ex) {
                System.out.println("Erro no Servlet Carregar Embalagem(Servlet)" + ex.getMessage());
            }
        } else {
