...
public class FuncionarioCarregar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (sessao != null) {
            try {
                int nidFuncipnario = Integer.parseInt(request.getParameter("idFuncionario"));
                FuncionarioDAO oFuncionarioDAO = new FuncionarioDAO();
                request.setAttribute("funcionario", oFuncionarioDAO.carregar(nidFuncipnario));
                request.getRequestDispatcher("alterarFuncionario.jsp").forward(request, response);
            } catch (Exception ex) {
                System.out.println("Erro no Servlet Carrega Funcionario(Servlet)" + ex.getMessage());
            }
        } else {
