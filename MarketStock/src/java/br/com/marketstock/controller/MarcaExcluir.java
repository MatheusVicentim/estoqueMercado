...
public class MarcaExcluir extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (sessao != null) {
            int nIdMarca = Integer.parseInt(request.getParameter("idMarca"));
            String sMensagem = "";
            try {
                MarcaDAO oMarcaDAO = new MarcaDAO();
                Marca oMarca = (Marca) oMarcaDAO.carregaInativaAtiva(nIdMarca);
                //Usado para abrir conexão com BD
                oMarcaDAO = new MarcaDAO();
                if (oMarcaDAO.excluir(oMarca)) {
                    sMensagem = "Marca " + oMarca.getSituacao() + " com sucesso!";
                } else {
                    sMensagem = "Problemas ao ativar ou excluir Marca. Verifique os dados informados e tente novamente!";
                }
