    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (sessao != null) {
            try {
                ...
                request.setAttribute("lFuncionarios", oFuncionarioDAO.listar());
                TipoPagamentoDAO oTipoPagamentoDAO = new TipoPagamentoDAO();
                request.setAttribute("lTiposPagamentos", oTipoPagamentoDAO.listar());
                request.getRequestDispatcher("/backEnd/pedidoCompraControle.jsp").forward(request, response);
            } catch (Exception ex) {
                System.out.println("Problemas no Servelet ao Pedido Compra! Erro: " + ex.getMessage());
            } 
        } else {
