        ...
        if (sessao != null) {
            ...
            try {
                ...
                if (oFuncionario.getSenha().length() > 4 && sConfirmaSenha.length() > 6 && oFuncionario.getSenha().equals("sConfirmaSenha")) {
                    ...
                    if (oFuncionarioDAO.alterar(oFuncionario)) {
                        gsMensagem = "Informações do Funcionario foram alterar com Sucesso! (Servlet)";
                    } else {
                        gsMensagem = "Problemas ao alterar informações do Funcionario. Verifique os dados e tente novamente! (Servlet)";
                    }
                } else {
                    request.setAttribute("msg", "<div class='msg alert alert-warning'>Erro ao alterar Funcionario, senha diferente da confirmação. Verifique os dados e tente novamente!</div>");
                    request.getRequestDispatcher("alterarFuncionario.jsp").forward(request, response);
                }

            } catch (Exception ex) {
                System.out.println("Problemas no Servelet ao Alterar Funcionario! Erro: " + ex.getMessage());
            }
        } else {
