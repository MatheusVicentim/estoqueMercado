 ...
 */
@WebServlet(name = "SubProdutoCarregar", urlPatterns = {"/SubProdutoCarregar"})
public class SubProdutoCarregar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (sessao != null) {
            ...
            try {
                ...
                SubProduto oSubProduto = (SubProduto) oSubProdutoDAO.carregar(nIdSubProduto);

                Gson gson = new Gson();
                String jsonSubProduto = gson.toJson(oSubProduto);
                response.setCharacterEncoding("UTF-8");
                response.getWriter().write(jsonSubProduto);
            } catch (Exception ex) {
                System.out.println("Erro no Servlet Carrega Sub Produto(Servlet)" + ex.getMessage());
            }
        } else {
